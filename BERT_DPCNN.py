...
class Config(object):
    ...
    def __init__(self, dataset):
        ...
        # self.sen_path = dataset + '/data/sen.txt'  # 预测
        self.class_list = [x.strip() for x in open(
            dataset + '/class.txt', encoding='utf-8').readlines()]  # 类别名单
        if not os.path.exists(dataset + '/saved_dict/'):
            os.mkdir(dataset + '/saved_dict/')
        self.save_path = dataset + '/saved_dict/' + self.model_name + '.ckpt'  # 模型训练结果
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # 设备

        self.require_improvement = 1000  # early_stop,若超过1000batch效果还没提升，则提前结束训练
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        encoder_out, text_cls = self.bert(context, attention_mask=mask)[:2]
